...
const ProgressBarWidget = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (response.data) {
          ...
          setProgressBarListData(
            response.data?.bars?.map((item, index) => ({
              id: index + 1,
  ...
  const handleMaintainButtonClick = (maintainValue) => {
    const updatedProgressBarListData = progressBarListData?.map((item) => {
      ...
      return {
        ...item,
        completed: item.selected ? updatedCompleted : item.completed,
      };
    });
    setProgressBarListData(updatedProgressBarListData);
  };
    ...
    <div className="widget">
      ...
      <>
        <div className="widget__bar">
          <ProgressBarList data={progressBarListData} />
        </div>
